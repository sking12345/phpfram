	...
	$.jstree.core.prototype = {
		...
		get_next_dom: function(obj, strict) {
			...
			if (strict) {
				...
				do {
					tmp = this._nextSibling(tmp)
				} while (tmp && tmp.offsetHeight === 0);return tmp ? $(tmp) : false
			}
			...
			do {
				tmp = this._nextSibling(tmp)
			} while (tmp && tmp.offsetHeight === 0);if (tmp !== null) {
				return $(tmp)
			}
		...
		_parse_model_from_html: function(d, p, ps) {
			...
			} while (m[tid]);data.id = data.li_attr.id ? data.li_attr.id.toString() : tid;
			if (tmp.length) {
