...
class ctl_index {
	...
	public function menus2() {
		$menu_xml_file = config::$obj->get("menus_xml_file");
		$xml_content = simplexml_load_file($menu_xml_file);
		$menus = $this->_menus_to_array2($xml_content, true, 0);
		tpl::assign("menus", $menus);
		tpl::assign("menus_children", $this->menus_children);
		tpl::display("index.menus2.tpl");
	}
	...
	public function _menus_to_array2($obj, $show_status, $top) {
		$menu_arr = [];
		foreach ($obj->children() as $key => $child_obj) {
			$item = ["id" => $this->id, "parent_id" => $top];
			$this->id++;
			...
			$children = $this->_menus_to_array($child_obj, $show_status, $item["id"]);
			if ($status != true) {
				$menu_arr[] = $item;
			}
			$this->menus_children[$item["id"]] = $children;
		}
		return $menu_arr;
	}
}
