            ...
            <div class="box-header with-border">
                <form method="GET" action="" class="form-inline">
                    ...
                    <select class="form-control " name="big_cate">
                       <script type="text/html" id="big_cate">
                               ...
                               {{each big_cate as item i}}
                                {{if item.code =="<?=snow\req::item('big_cate')?>"}}
                                    {{if item.unit == "<?=snow\req::item('unit_list')?>"}}
                                    <option class="unit{{item.unit}} unit " value="{{item.code}}" selected="true">{{item.name}}</option>
                                    {{else}}
                                 <option class="unit{{item.unit}} unit hide" value="{{item.code}}" selected="true">{{item.name}}</option>
                                    {{/if}}
                                {{else}}
                                    {{if item.unit == "<?=snow\req::item('unit_list')?>"}}
                                   <option class="unit{{item.unit}} unit " value="{{item.code}}">{{item.name}}</option>
                                    {{else}}
                                  <option class="unit{{item.unit}} unit hide" value="{{item.code}}">{{item.name}}</option>
                                    {{/if}}
                                {{/if}}
                               {{/each}}
                        </script>
                    ...
                    <div class="form-group  pull-right">
                        <a href="?ctl=<?=snow\req::get('ctl')?>&act=add_small_cate" class="btn  btn-info"><i class="fa fa-plus-circle"></i>添加小类</a>
                    </div>
                </form>
            </div>
            <div class="box-body">
                    ...
                    <tbody>
                        ...
                        <tr>
                              ...
                              <td>{{kinds[item.kinds].name}}</td>
                               ...
                               <td>{{big_cate[item.big_cate].name}}</td>
                              <td>{{item.code}}</td>
                              <td>
                                <a href="?ctl=<?=snow\req::get('ctl')?>&act=edit_small_cate&id={{item.id}}" class="btn btn-primary  btn-sm"><i class="fa fa-fw fa-edit"></i>编辑</a>
        ...
        $(document).ready(function() {
            ...
            $("select[name=kind]").change(function(){
             ...
             $("select[name=unit_list]").change(function(){
                $("select[name=big_cate]").find("option[value=' ']").attr("selected",true);
                var kind_val =  $("select[name=unit_list]").find("option:selected").val();
                 $(".unit").addClass("hide");
                $(".unit"+kind_val).removeClass("hide");
            });
        });
        </script>
</body>
